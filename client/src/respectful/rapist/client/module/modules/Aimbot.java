...
public class Aimbot extends Module {
    public float maxYawSmooth = 10.5F, minYawSmooth = 10.0F, maxPitchSmooth = 12.5F, minPitchSmooth = 12.0F, dist = 4.0F, FOV = 45.0F, randX, randY, randZ;
    ...
    public void onTick() {
        if (Config.safe(reqItem, itemWhitelist, reqMouse)) {
            ...
            if (target != null && Mappings.EntityLivingBase.isEntityAlive(Mappings.EntityPlayer.clazz.cast(target))) {
                double x = Mappings.Entity.getPosX(target) - Mappings.Entity.getPosX(Mappings.Minecraft.getThePlayer());
                double y = Mappings.Entity.getPosZ(target) - Mappings.Entity.getPosZ(Mappings.Minecraft.getThePlayer());
                double z = Mappings.Entity.getPosY(target) - Mappings.Entity.getPosY(Mappings.Minecraft.getThePlayer()) + 1;
                ...
                float yaw = (float) ((Math.atan(randY / randX)) * (180.0F / Math.PI));
                float pitch = (float) (-1 * ((Math.asin(randZ / Mappings.Entity.getDistanceToEntity(Mappings.Minecraft.getThePlayer(), Mappings.EntityPlayer.clazz.cast(target))) * (180.0F / Math.PI))));
                yaw += (randX < 0.0D) ? 90.0F : -90.0F;
                yaw = findNormalizedAngle(yaw);
                float neededRotationYaw = yaw - findNormalizedAngle(Mappings.Entity.getRotationYaw(Mappings.Minecraft.getThePlayer()));
                float neededRotationPitch = pitch - Mappings.Entity.getRotationPitch(Mappings.Minecraft.getThePlayer());
                float coterminal = findCoterminalAngle(neededRotationYaw);
